    ...
    if (f_nf->is_open()) {
        ...
        while (!f_nf->eof()) {
            ...
            for (int j = 0; j < seq.size() - k + 1; j++) {
                ...
                else { am->kmerFreq->insert(make_pair(kmer, am->kmerFreq->at(kmer) + 1)); }
                ...
                for (int a : *positions) {
                    if (j <= a && a - k + 1 <= j) {
                        if (mutFromkmer->count(kmer)) {
                            ...
                            muth.push_back(id);
                            mutFromkmer->insert(make_pair(kmer, muth));
                        }
...
void Parsing::Nucio()
{
    ...
    string filename("/mnt/user/0/emulated/0/Download/neclo.txt");
...
void Parsing::Qread(const string& filename, const bool& reportMultipleHits)
{
    ...
    fout->open("/mnt/user/0/emulated/0/Download/_KARGVA_mappedReads.csv", ios::out | ios::app);
    ...
    *fout << "\n";
                    ...
                    for (int b = 0; b < stp; b++) {
                        ...
                        for (int c = 0; c < kmerHitsBest->size(); c++) {
                            ...
                            if (genehit.kmerFreq->count(kh)) {
                                ...
                                else { genehit.kmerMapped->insert(make_pair(kh, genehit.kmerMapped->at(kh) + 1)); }
                            }
    ...
    f_ot->open("/mnt/user/0/emulated/0/Download/_KARGVA_mappedGenes.csv", ios::out | ios::app);
